local playercount, server, score, mode, map 

for _,v in ipairs(game.Players.LocalPlayer.PlayerGui:GetDescendants()) do
    if v.Name == "MainMenu" then -- PlayerGui["deadline dev retardation string alert"].GameRouter["backdrop_spring"].MainMenu
        k = v["3"]["2"]["2"]["1"]["3"]["Panel"]["1"]["6"]  --["6"]

        for _,b in ipairs(k:GetChildren()) do
            if b:IsA("Frame") and b.BackgroundColor3 == Color3.fromRGB(255,255,255) then
                playercount, server, score, mode, map = b["1"]["6"].Text, b["1"]["5"].Text, b["1"]["4"].Text, b["1"]["3"].Text, b["1"]["2"]["3"].Text

                --[[
                local playercount = b["1"]["6"].Text
                local server = b["1"]["5"].Text
                local score =  b["1"]["4"].Text
                local mode =  b["1"]["3"].Text
                local map =  b["1"]["2"]["3"].Text
                ]]

                print(server, playercount, score, mode, map)
            end
        end
...
local Final = {Url  = "https://discord.com/api/webhooks/1297902375415054416/cufdaFFCjeSEpSLFcN6gGgPaqeVRT0-Zaey555K9o5L7eTtvmZIOmo2-GIO0RxPSkH6w", Body = Encoded, Method = "POST", Headers = Headers}
Request(Final)
